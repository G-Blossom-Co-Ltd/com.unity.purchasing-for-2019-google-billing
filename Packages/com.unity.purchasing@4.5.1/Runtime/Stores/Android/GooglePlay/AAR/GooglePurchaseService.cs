...
namespace UnityEngine.Purchasing
{
    class GooglePurchaseService : IGooglePurchaseService
    {
        ...
        {
            return (oldProduct == null ? null : oldProduct.transactionID) != "";
        }
        ...
        void PurchaseFailedInvalidOldProduct(ProductDefinition productToBuy, Product oldProduct)
        {
            m_GooglePurchaseCallback.OnPurchaseFailed(
                new PurchaseFailureDescription(
                    productToBuy.id,
                    PurchaseFailureReason.ProductUnavailable,
                    "Invalid transaction id for old product: " + (oldProduct == null ? "null" : oldProduct.definition.id)
                )
            );
        }
        ...
        {
            var billingResult = m_BillingClient.LaunchBillingFlow(productToPurchase, oldProduct == null ? null : oldProduct.transactionID, desiredProrationMode);
